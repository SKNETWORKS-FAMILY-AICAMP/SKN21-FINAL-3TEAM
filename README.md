...
**윤경은**
 ...
 판단 Agent의 서빙 안정성을 확보하는 과정에서 가장 많이 배웠습니다.  
sLLM(Kanana-1.5-8B)이 confidence 0.95를 출력하면서도 존재하지 않는 조항을 인용하는 환각 문제를 마주했을 때, 모델 성능 자체를 높이는 것보다 서버에서 출력을 검증·보정하는 구조가 더 효과적이라는 판단을 내렸습니다. 4중 Guardrail(규정 키워드 매칭, 조항 존재 검증, 카테고리 제한, 일관성 모니터링)과 5-factor Confidence 보정 공식을 설계하여 판단 정확도를 37%에서 85%로, JSON 유효율을 70%에서 97%로 끌어올렸습니다. RAG 파이프라인도 BM25+Qdrant 하이브리드 검색에 HyDE·RRF·Reranker를 결합한 9단계로 구축하여 MRR을 0.63에서 0.95로 개선했습니다. 특히 LoRA v2에서 학습 데이터 98건을 무작위로 추가했더니 라벨 오염으로 오히려 -3.2%p 하락한 경험이 결정적이었는데, v3에서는 약점 19건(재량 표현 14건 + 경계 케이스 5건)만 정밀 타겟팅하여 성능을 회복했습니다. "데이터 양보다 데이터 질"이라는 원칙을 몸소 체감한 순간이었습니다.

아쉬운 점은 vLLM 서빙 환경에서의 성능 프로파일링을 깊이 있게 하지 못한 것입니다. RunPod A100에서 학습과 서빙을 동시에 진행하면서 GPU 메모리 관리나 배치 추론 최적화 같은 서빙 인프라 레벨의 튜닝까지는 시간이 부족했습니다. 또한 공통 LLM 모듈을 설계하여 GPT/Claude/vLLM 간 provider 전환은 구현했지만, 각 provider별 추론 속도 벤치마크나 비용 대비 성능 비교를 체계적으로 기록하지 못한 점도 아쉽습니다. 그럼에도 이번 프로젝트를 통해 "외부 시스템(LLM)의 출력을 서버가 검증·보정하는 설계 원칙"과 "sLLM 서빙의 핵심은 모형 성능 자체보다 서빙 안정성(JSON 파싱, 에러 핸들링, fallback)에 있다"는 두 가지 판단 기준을 확실히 얻었습니다.
