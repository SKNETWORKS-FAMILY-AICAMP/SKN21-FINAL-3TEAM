    ...
    <div>
      <header className="flex justify-between items-center py-6 sticky top-0 bg-surface-main z-10">
        <div>
          ...
          <p className="text-sm text-neutral-sub mt-1">안녕하세요, 김정보님! 오늘의 현황을 확인하세요.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-surface-card border border-neutral-border rounded-md px-4 py-2 min-w-[280px]"><span>🔍</span><input type="text" placeholder="규정 검색 (정보보안, 인사, 개발)" className="border-none bg-transparent text-[13px] w-full outline-none placeholder:text-neutral-muted" /></div>
          <button className="w-10 h-10 rounded-sm border border-neutral-border bg-surface-card flex items-center justify-center relative"><span>🔔</span><span className="absolute top-2 right-2 w-2 h-2 bg-error rounded-full border-2 border-surface-card" /></button>
        </div>
      </header>
