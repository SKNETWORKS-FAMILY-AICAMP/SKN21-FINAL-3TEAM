...
export default function DocumentUpload({ onUpload, onScopeChange }) {
  return (
    <div className="border-2 border-dashed border-neutral-border rounded-md min-h-[280px] flex flex-col items-center justify-center text-center transition hover:border-primary-300 hover:bg-primary-50 cursor-pointer mb-5"
      onClick={() => document.getElementById('file-upload')?.click()}>
      <input type="file" id="file-upload" className="hidden" accept=".pdf,.docx,.txt" onChange={(e) => onUpload?.(e.target.files)} />
      <div className="mb-2"><Upload size={32} className="text-neutral-muted" /></div>
