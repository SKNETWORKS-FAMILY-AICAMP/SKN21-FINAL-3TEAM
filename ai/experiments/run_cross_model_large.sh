#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# 크로스 모델 실험 v2 — Large 모델 후보
#
# 목적: roberta-large 외 다른 아키텍처(Large급)를 학습하고
#       크로스 모델 앙상블로 93.3% → 95%+ 가능한지 확인
#
# 사용법 (RunPod):
#   cd /workspace/SKN21-FINAL-3TEAM
#   bash ai/experiments/run_cross_model_large.sh
# ═══════════════════════════════════════════════════════════════

set -e

echo "═══════════════════════════════════════════════════════"
echo "  크로스 모델 실험 v2 — Large 모델"
echo "═══════════════════════════════════════════════════════"

# GPU / 패키지 확인
python3 -c "import torch; print(f'GPU: {torch.cuda.get_device_name(0)}'); print(f'VRAM: {torch.cuda.get_device_properties(0).total_memory/1e9:.1f}GB'); print(f'PyTorch: {torch.__version__}')"
...
models = [
    ...
    ('xlm-roberta-large', 'holdout_xlm-roberta-large.json'),
    ('microsoft_deberta-v3-large', 'holdout_microsoft_deberta-v3-large.json'),
    ('beomi_KcBERT-large', 'holdout_beomi_KcBERT-large.json'),
]
...
echo "다음 단계:"
echo "  1. 가장 좋은 모델 확인"
echo "  2. roberta-large 5-seed 앙상블 + 최고 large 모델로 크로스 앙상블"
echo "  3. 최고 모델도 5-seed 앙상블 학습 → 크로스 앙상블 평가"
