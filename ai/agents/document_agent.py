...
def _build_search_prompt(query: str, context: list) -> tuple:
    ...
    [중요 지시사항]
    - 사용자의 검색 키워드와 실제로 관련 있는 문서만 나열하세요
    - 관련 없는 문서는 절대 나열하지 마세요
    - 관련 있는 문서가 하나도 없으면 "관련 문서를 찾지 못했습니다. 다른 키워드로 검색해보세요."라고만 답하세요
    - 관련 있는 문서가 있으면 "다음 문서들을 찾았습니다:" 형식으로 시작하고, 각 문서의 제목과 핵심 내용을 한 줄로 요약하세요
    - 관련 있는 문서는 빠뜨리지 마세요
