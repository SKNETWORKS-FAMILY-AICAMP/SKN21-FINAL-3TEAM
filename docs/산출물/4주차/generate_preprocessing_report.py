...
add_title('인공지능 데이터 전처리 결과서', size=18)
doc.add_paragraph()
...
add_subheading('1-2. Intent 카테고리 (8개)')
...
make_table(
    ...
    [
        ...
        ['시나리오 테스트셋', '100건', '실서비스 시나리오 정성 평가', 'Gemini Pro 3.1 생성\n+ 수동 검수'],
    ]
)
...
# 2. 원본 데이터 샘플
# =============================================
add_heading('2. 원본 데이터 샘플')

add_body('각 데이터 유형별 원본 샘플을 아래에 제시합니다. 모든 데이터는 동일한 JSONL 형식(text + label)입니다.')

add_subheading('2-1. 기본 데이터 샘플 (8개 intent, 각 1건)')
...
add_subheading('2-2. 경계 쌍 데이터 샘플 (혼동 가능 intent 쌍)')
...
add_subheading('2-3. Adversarial 테스트 데이터 샘플 (모델 취약 유형)')
...
# 3. 전처리 흐름도
# =============================================
add_heading('3. 전처리 흐름도')

add_body(
    'Intent Classifier v2의 전처리는 크게 '
    '데이터 생성 -> 품질 검증 -> 텍스트 전처리 -> 분할의 4단계로 구성됩니다.'
)

add_subheading('3-1. 전체 파이프라인')

flow = (
    "+-------------------------------------------------------------+\n"
    "|  Step 1: 데이터 생성 (generate_data.py)                     |\n"
    "|                                                             |\n"
    "|  GPT-4o / Claude Sonnet 4 -> 기본 2,299건                  |\n"
    "|  + 경계 쌍 600건 + Adversarial 450건                       |\n"
    "|  + 교차 검증 (LLM A 생성 -> LLM B 라벨 검증)              |\n"
    "+----------------------------+--------------------------------+\n"
    "                             v\n"
    "+-------------------------------------------------------------+\n"
    "|  Step 2: 품질 검증 (QA)                                     |\n"
    "|                                                             |\n"
    "|  (1) 중복 제거 (exact text match) -> 59건 제거             |\n"
    "|  (2) 라벨 유효성 (8개 intent 이외 라벨 탐지) -> 0건        |\n"
    "|  (3) 클래스 균형 (max/min ratio <= 1.2) -> 1.11 통과       |\n"
    "|  (4) 데이터 누출 (Train<->Val<->Test 중복) -> 0건          |\n"
    "+----------------------------+--------------------------------+\n"
    "                             v\n"
    "+-------------------------------------------------------------+\n"
    "|  Step 3: 텍스트 전처리 (preprocessing.py)                   |\n"
    "|                                                             |\n"
    "|  P4 공백/특수문자 정리 -> P1 맞춤법 교정                   |\n"
    "|  -> P2 초성 복원 -> P3 슬랭 정규화                         |\n"
    "|                                                             |\n"
    "|  * Ablation 실험 결과: 전처리 유무 성능 차이 없음           |\n"
    "|  * 서비스 안정성을 위해 P4(공백 정리)만 기본 적용           |\n"
    "+----------------------------+--------------------------------+\n"
    "                             v\n"
    "+-------------------------------------------------------------+\n"
    "|  Step 4: 데이터 분할 (Stratified Split)                     |\n"
    "|                                                             |\n"
    "|  Train 2,425건(80%) / Val 285건(10%) / Test 286건(10%)     |\n"
    "|  seed=42, intent별 비율 유지                               |\n"
    "+----------------------------+--------------------------------+\n"
    "                             v\n"
    "+-------------------------------------------------------------+\n"
    "|  Step 5: 토크나이징 (Tokenizer)                             |\n"
    "|                                                             |\n"
    "|  koelectra-base-v3 WordPiece tokenizer                     |\n"
    "|  max_length=64, padding='max_length', truncation=True      |\n"
    "+-------------------------------------------------------------+"
)
add_code(flow)

add_subheading('3-2. 실험 기반 보강 루프 (Stage 5~7)')
...
flow2 = (
    ...
    "     |\n"
    "     v\n"
    "타겟 보강 98건 생성 (Stage 5) -> Train에 합산 -> 재학습\n"
    "     |\n"
    "     v\n"
    "Label Smoothing 0.1 적용 (Stage 6) -> 과신뢰 오분류 69% 감소\n"
    "     |\n"
    "     v\n"
    "최종 모델: Test F1 97.88% / Adv F1 87.84%"
)
add_code(flow2)
...
# 4. 세부 전처리 단계
# =============================================
add_heading('4. 세부 전처리 단계')

# 4-1
add_subheading('4-1. 결측치 제거 및 라벨 유효성 검증')
...
make_table(
    ['검증 항목', '검증 방법', '결과'],
    [
        ['text 필드 결측', 'JSONL 라인별 text 키 존재 + 빈 문자열 여부 체크', '결측 0건'],
...
# 4-2
add_subheading('4-2. 중복 제거')

add_body('텍스트 해시 기반 완전 중복(exact match)을 제거하였습니다.')
...
add_body('Adversarial 데이터에서도 Train과의 중복 제거 적용: 463건 -> 450건 (13건 중복 제거)')

# 4-3
add_subheading('4-3. 클래스 균형 검증 (정규화)')
...
add_note('* 경계 쌍 600건은 10개 쌍 x 30건 x 2 LLM, 각 intent에 균등 배분')
add_note('* 최종 Train 기준 클래스 균형: max/min ratio = 1.28x (양호)')

doc.add_page_break()

# 4-4
add_subheading('4-4. 텍스트 전처리 (preprocessing.py)')

add_body(
    '4단계 규칙 기반 전처리 파이프라인을 구현하였습니다. '
    '각 단계는 개별 on/off가 가능하며, Ablation 실험으로 효과를 검증하였습니다.'
)

add_body('실행 순서: P4(공백 정리) -> P1(맞춤법) -> P2(초성 복원) -> P3(슬랭 정규화)')

# P4
p = doc.add_paragraph()
run = p.add_run('P4. 공백/특수문자 정리 (Text Cleaning)')
run.bold = True
run.font.size = Pt(10)
...
# P1
p = doc.add_paragraph()
run = p.add_run('P1. 맞춤법 교정 (Spell Check)')
run.bold = True
run.font.size = Pt(10)
...
# P2
p = doc.add_paragraph()
run = p.add_run('P2. 초성 복원 (Chosung Restoration)')
run.bold = True
run.font.size = Pt(10)
...
# P3
p = doc.add_paragraph()
run = p.add_run('P3. 슬랭 정규화 (Slang Normalization)')
run.bold = True
run.font.size = Pt(10)
...
# 4-5
add_subheading('4-5. 전처리 Ablation 실험 결과')
...
p = doc.add_paragraph()
run = p.add_run('-> 결론: 서비스 안정성을 위해 P4(공백 정리)만 기본 적용. 전처리보다 데이터 품질이 성능에 결정적.')
run.bold = True
run.font.size = Pt(9)
...
# 4-6
add_subheading('4-6. 데이터 분할 (Stratified Split)')
...
add_body('데이터 누출 검증: Train<->Val 중복 0건, Train<->Test 중복 0건, Val<->Test 중복 0건')

# 4-7
add_subheading('4-7. 토크나이징 (Tokenizer)')
...
# 4-8
add_subheading('4-8. 라벨 인코딩 (Label Encoding)')
...
# 5. 전처리 결과 요약
# =============================================
add_heading('5. 전처리 결과 요약')

add_subheading('5-1. 전처리 전후 비교 (샘플)')
...
add_subheading('5-2. 최종 데이터 파일 구조')

add_code(
    'data/training/intent_v2/\n'
    '|-- raw/                        # LLM별 원본 (GPT/Claude x 8 intent)\n'
    '|-- {intent}.jsonl x 8          # intent별 정제 데이터 (중복 제거 후)\n'
    '|-- boundary_pairs.jsonl        # 경계 쌍 600건\n'
    '|-- adversarial_v2.json         # Adversarial 450건\n'
    '|-- scenario_test.json          # 시나리오 100건\n'
    '|-- augmentation_stage5.jsonl   # Stage 5 보강 98건\n'
    '|-- splits/\n'
    '|   |-- train.jsonl             # 2,425건 (학습용)\n'
    '|   |-- val.jsonl               # 285건 (검증용)\n'
    '|   |-- test.jsonl              # 286건 (평가용)\n'
    '|-- DATA_QA_REPORT.md           # 품질 검증 보고서 (자동 생성)'
)

add_subheading('5-3. 사용 도구')
...
out_path = os.path.join(out_dir, '2.데이터 전처리_인공지능 데이터 전처리 결과서_3팀.docx')
doc.save(out_path)
