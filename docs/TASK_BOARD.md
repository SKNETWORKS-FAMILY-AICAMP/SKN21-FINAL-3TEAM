...
| 이름 | 역할 | GitHub 라벨 | 비고 |
|------|------|-------------|------|
| **신지용** | PM + Intent 분류 + 문서 Agent + 오케스트레이션 + AWS 배포 | `지용:PM` | 승언 업무 인수 |
| **윤경은** | AI (파인튜닝 v1 + 판단 Agent + RAG + vLLM 서빙) | `경은:AI서브` | |
| ~~진승언~~ | ~~AI 리드 (파인튜닝 v2 + 문서 Agent)~~ | `승언:AI리드` | **3월부터 불참** |
| **안혜빈** | Backend + 팀서비스 확장 (Slack/Jira) + vLLM 연동 | `혜빈:Backend` | |
| **문지영** | Frontend + UI/UX 개선 + 팀서비스 프론트 | `지영:Frontend` | |
...
[지용] LangGraph Agent Orchestrator
     ...
     │
     ├── doc_search     → [지용] RAG + 하이브리드검색 → 정리된 답변 (승언 인수)
     ├── doc_generate   → [지용] 템플릿 기반 문서 생성 (회의록 DOCX) (승언 인수)
     ├── doc_summary    → [지용] 문서 요약 (스트리밍) (승언 인수)
     ├── doc_qa         → [지용] 문서 QA (citations + confidence) (승언 인수)
     │
...
| #67 | **[C-9] 문서 템플릿 기반 생성 완성** | 보고서/제안서 render() 구현, generate API 501 해제 | ✅ |
...
| #68 | **[E-8] 챗봇 문서 응답 카드 UI 개선** | doc_generate 카드, doc_qa citations/confidence 표시 | ✅ |

---

# 4단계: 데이터 수집 + 파인튜닝 🔄 (진행중)

> 중간발표 완료. LLM API로 기능 검증 완료 → sLLM 파인튜닝 단계 진입
> ⚠️ 승언 불참으로 문서 Agent 관련 작업을 **지용이 인수**

---

### 윤경은 — 판단 Agent 파인튜닝 (1~2주차)

| # | 이슈 | 할 일 | 상태 |
|---|------|-------|:----:|
| #9 | **판단 데이터 변환** | Excel → JSONL + 규정 원문 추가 | 📋 |
| #10 | **LoRA v1 파인튜닝** | 판단 + Q&A, QLoRA 4-bit | 📋 |

### 윤경은 — vLLM 서빙 + sLLM 교체 (3주차)
...
### 신지용 — 문서 Agent 데이터 수집 + LoRA v2 (1~2주차, 승언 인수)

| # | 이슈 | 할 일 | 상태 |
|---|------|-------|:----:|
| #14 | **학습 데이터셋 구축** | 문서 생성/요약/QA 데이터 수집 | 📋 |
| #41 | **추가 데이터 수집** | 문서 요약 + 문서 생성 + 검색 답변 + 리스크 | 📋 |
| #16 | **LoRA v2 파인튜닝** | 문서 특화, 경은과 동일 베이스 모델 | 📋 |

### 신지용 — Intent 분류 강화 (3주차)

| # | 이슈 | 할 일 | 상태 |
|---|------|-------|:----:|
| #79 | **Intent 분류 고도화** | KoELECTRA multi-label (복합질문 감지 → 사용자 선택 UI) | 📋 |
...
| **경은** | #13 | 성능 평가 리포트 (판단 정확도, RAG MRR, 응답속도) | 📋 마지막주 |
| **지용** | #18 | 성능 평가 리포트 (ROUGE-L, BERTScore, F1) — 승언 인수 | 📋 마지막주 |
| **지용** | #83 | sLLM 교체 후 E2E 재검증 테스트 | 📋 3주차 |
...
| 담당 | # | 할 일 | 시기 |
|------|---|-------|------|
| **지용** | #80 | Frontend 배포 (Vite + S3/CloudFront) | 3주차 |
| **지용** | #81 | 최종 QA + E2E 통합 테스트 | 마지막주 |
| **전원** | #82 | 최종 발표 준비 + 성능평가리포트 | 마지막주 |
...
> 승언 불참 → 문서 Agent + 데이터 수집 + LoRA v2 + AWS 배포를 **지용이 인수**
> 복합질문 처리: **KoELECTRA multi-label + 사용자 선택 UI** (방법 2)

## 1~2주차 (3/3~3/14)

| 담당 | 핵심 작업 | 관련 이슈 |
|------|----------|----------|
| **경은** | 판단 Agent 데이터 수집 + LoRA v1 | #9, #10 |
| **지용** | 데이터 수집 + 문서 Agent (생성/요약/QA) LoRA v2 | #14, #41, #16 |
| **혜빈** | 팀서비스 확장 (Slack, Jira 연동) + UI/UX 수정 | #84 |
| **지영** | 팀서비스 확장 프론트 + UI/UX 수정 | #85 |

## 3주차 (3/15~3/23)

| 담당 | 핵심 작업 | 관련 이슈 |
|------|----------|----------|
| **경은** | vLLM 서빙 + sLLM 교체 및 평가 | #11, #13 |
| **지용** | Intent 분류 강화 (단일→복합질문, multi-label KoELECTRA) | #79 |
| **혜빈** | vLLM 백엔드 연동 + sLLM 교체 및 평가 | #86 |
| **지영** | 팀서비스 기능 운영 + UI 최종 대응 | #87 |

## 마지막주 (3/24~3/31)

| 담당 | 핵심 작업 | 관련 이슈 |
|------|----------|----------|
| **전원** | 발표 준비 + 성능평가 리포트 | #82, #13, #18 |
| **지용** | 최종 QA + E2E 통합 테스트 | #81, #83 |
...
| 데이터 | 어댑터 | 상태 |
|--------|--------|:----:|
| 규정 판단 (Yes/No) | LoRA v1 | ✅ Excel → JSONL 변환 필요 |
| 회의록 분석 | LoRA v2 | ✅ JSON → JSONL 변환 필요 |
| 규정 Q&A | LoRA v1 | 🔄 수집 중 |
...
| 데이터 | 어댑터 | 담당 |
|--------|--------|------|
| Intent 분류 문장 (8개 카테고리) | BERT | 지용 |
| 문서 검색 답변 | LoRA v2 | 지용 |
| 문서 요약 (동적 필드) | LoRA v2 | 지용 |
| 문서 생성 (동적 필드) | LoRA v2 | 지용 |
| 리스크 감지 | LoRA v2 | 지용 |
...
| 어댑터 | 데이터 | 담당 |
|--------|-------|------|
| **LoRA v1** (판단 특화) | 판단 + Q&A | 경은 |
| **LoRA v2** (문서 특화) | 회의록 + 검색 + 요약 + 생성 + 리스크 | 지용 (승언 인수) |

> 건수는 수집 후 확정 / 검증용 15% 별도 분리 (학습에 사용하지 않음)
