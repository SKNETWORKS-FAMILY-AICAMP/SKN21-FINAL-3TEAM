...
#### 22) 인사 메시지 사용법 버튼 표시 수정 (`ChatPage.jsx`)

- **문제**: "안녕하세요" 보내도 사용법 버튼이 안 나옴
- **원인 1**: `isFirstAssistant` 조건 — 기존 대화에서는 첫 assistant가 아니라 판정되어 버튼 미표시 → 조건 제거
- **원인 2**: 정규식 `\b`(word boundary)가 한글에서 작동 안 함 — `"안녕하세요"`가 `"안녕"`으로 매칭 실패 → `\b` 제거
- **추가**: "사용법 알려줘", "도움말", "뭐 할 수 있어", "어떻게 써" 등 패턴도 사용법 버튼 표시 대상에 추가

#### 23) 인사 응답 고정 텍스트 + UI 개선 (`ChatPage.jsx`)

- LLM이 매번 다른 인사 응답을 생성하는 문제 → 인사일 때 고정 텍스트("안녕하세요! 듀드입니다. / 오늘도 업무에 도움이 필요하신가요? / 궁금한 점이나 요청 사항을 말씀해 주세요.")로 교체
- "(업무와 관련된 질문만 부탁드립니다.)" 문구 프론트에서 제거
