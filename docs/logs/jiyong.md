...
**커스텀 템플릿 파이프라인 구현 (추출기 v2 + 한글 키 + fill-fields API + 프론트 UI)**

한 일:
- **추출기 v2 재작성** (`template_extractor.py`): 구조 기반 다열 스캔, 플레이스홀더 감지, 번호 접두사 제거, 미매핑 라벨 커스텀 키 자동 생성
...
- **2단계 분기 제거**: trained/untrained 분리 삭제, LoRA 1회로 전체 필드 생성 (TRAINED_KEYS, _extract_structured_fields 죽은 코드 삭제)
- **fill-fields API 추가** (`POST /documents/fill-fields`): DOCX 생성 없이 LoRA로 필드 값만 리턴, 12개 필드 제한 내장
- **프론트 커스텀 템플릿 UI**: 자연어 입력 → "AI가 필드 채우기" → 모든 필드 편집 가능 → 문서 생성 플로우
- 시스템 템플릿(영어 키) / 커스텀 템플릿(한글 키) 경로 분리 확인 → DOCX 빌더 호환 문제 없음
- E2E Playwright 테스트: 로그인 → 문서 생성 페이지 → 시스템/커스텀 템플릿 선택 → UI 렌더링 확인

다음 할 일:
- 제안서, 보고서 등 다른 문서 유형의 커스텀 양식 추출 + 생성 테스트
- 커스텀 양식 DOCX 다운로드 검증 (프록시 설정 이슈 해결 후)
- 짧은 입력 할루시네이션 개선 (LoRA 학습 데이터 보강)
- 커스텀 필드 description 품질 개선 (자동 생성 로직)
