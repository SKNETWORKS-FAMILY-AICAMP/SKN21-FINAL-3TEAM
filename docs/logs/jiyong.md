...
## 2026-03-28 (금) — 발표자료 v2 전면 재구성

**목표:** presentation_v2.html 발표 15분 기준 전면 재구성 (섹션 구조 + 디자인 + 내용 검증)

### 한 일

1. **서론 섹션 전면 교체 (깡이.html 반영)**
   - 기존 01~03 (Why/솔루션/sLLM) 삭제 → 깡이.html 페르소나 기반 도입부로 교체
   - 01 경영지원팀의 하루 (Before/After) → 02 GPT를 쓸 수 없는 이유 → 03 DUDE = 유일한 해법
   - 04 DUDE 핵심 기능 신규 생성 (3 Agent 카드 + 부제에 AI 챗봇 기능)

2. **섹션 구조 대폭 정리**
   - RAG 파이프라인(09-R) 삭제 → 07-1 Judgment Agent에 RAG 한 줄 흡수
   - 트러블슈팅(11) + 한계점(12) → 12번으로 합침
   - 마일스톤 삭제
   - Agent 구조 개요 → Intent 분류(08)로 교체 (Agent 3번 반복 문제 해결)
   - 파인튜닝 성과 6장 → 통합표 1장 + 상세 4장

3. **아키텍처 섹션(07) 재디자인**
   - 코드명(classify_intent 등) → 한글/적절한 영어 혼합
   - 좌우 패널에 CLIENT/SERVICES 라벨 추가, 화살표 정리
   - 카드형 레이아웃으로 높이 맞춤

4. **Agent 상세(08-1~3) 양식 통일**
   - 3개 모두 좌우 2단 (왼: 파이프라인 / 우: 핵심 포인트 + 차별점)
   - 코드명 제거, 발표용 한글화
   - Judgment: JSON 출력 → 핵심 차별점으로 교체

5. **기술 스택(06) 디자인 개선**
   - 리스트 항목에 뱃지(bg-primary-50) 적용, 색상 통일

6. **Base 모델 한계(09) 개선**
   - 판단 Agent JSON → 텍스트 형식으로 통일 (3카드 양식 맞춤)
   - 문서생성 구체적 예시("budget": "연간 5억 원") 추가
   - ROUGE-L, BERTScore 제거 (11번 성과 비교에서 보여줄 수치)

7. **12번(트러블슈팅 & 한계점) v4 실험 반영**
   - 한계점: "ROUGE-L ↑ vs False Fill ↑ 트레이드오프" 추가
   - 향후 방향: "학습 데이터 확장 + 더 큰 모델" 추가

8. **목차/nav dots/JS 전체 동기화**
   - 섹션 변경에 맞춰 목차, nav dots, IntersectionObserver ids 전부 업데이트
   - 서브 항목(08-1~3, 11-1~4) 목차에 들여쓰기 스타일로 추가

### 최종 섹션 구조
```
01 경영지원팀의 하루 | 02 GPT를 쓸 수 없는 이유 | 03 DUDE = 유일한 해법
04 DUDE 핵심 기능 | 05 팀 구성 | 06 기술 스택
07 전체 시스템 아키텍처 | 08 Intent 분류 | 08-1~3 Agent 상세
09 Base 모델 한계 | 10 데이터셋 현황
11 sLLM 파인튜닝 성과 비교 | 11-1~4 상세
12 Trouble Shooting & 한계점 | Q&A
```

### 다음 할 일
- 11번(파인튜닝 성과 통합표 + 상세 4장) 내용/디자인 검토
- 데모 영상 5분 촬영
- 전체 발표 리허설 (15분 시간 체크)

---

## 2026-03-27 (목) — v3 generate/summary 성능 개선 실험

**목표:** v3_generate, v3_summary 파인튜닝 성능 향상 (데이터 추가 생성 없이)

### 한 일

1. **v3_generate 학습결과 재분석**
   - 기존 분석이 "채움률 하락 = 문제"로 잘못 해석되어 있었음
   - 실제로는 v3 설계 의도(근거 기반 판단)대로 **정상 동작** → 문서 수정
   - 진짜 개선 포인트: JSON 유효율(87.3%), False Fill(17.9%)

2. **토큰 분석 (RunPod H200)**
   - eval 데이터 150건 assistant 출력: 평균 653토큰, 1024 초과 2%뿐
   - `max_new_tokens=1024`는 Kanana 토크나이저 특성상 충분 → 출력 잘림 가설 기각

3. **v4_generate 실험 (r=32 유지, LR=5e-5, epochs=10, save_steps=25)**
   - Best: step 275 (epoch 2.95), eval_loss=0.5110 (v3: 0.508과 동일)
   - 결과: ROUGE-L 0.665→**0.704** ✅, JSON 유효율 87.3%→**92.0%** ✅
   - **하지만 False Fill 17.9%→26.6% 악화** ❌ → 근거 판단 능력 훼손
   - ROUGE-L vs False Fill 트레이드오프 확인 → **v4 기각, v3 유지**

4. **v4_summary 실험 (r=16 유지, LR=5e-5, epochs=10, save_steps=25)**
   - Best: step 250 (epoch 3.96), eval_loss=0.9950 (v3: 0.9910과 동일)
   - 결과: BERTScore 0.8594→0.8573 (-0.002), 태그 적정 81%→92%
   - 핵심 지표 변화 없음 → **v4 기각, v3 유지**

5. **문서 업데이트**
   - `docs/finetuning/v3_generate_학습결과.md` — 의도된 동작 반영 + v4 실험 결과 추가
   - `docs/finetuning/v3_summary_학습결과.md` — v4 실험 결과 + "v3이 천장" 결론 추가
   - `docs/plans/V3_GENERATE_IMPROVEMENT_PLAN.md` — v2로 전면 재작성 (올바른 분석 기반)

### 핵심 결론
- **generate/summary 모두 v3 어댑터가 최적** — 하이퍼파라미터 튜닝으로는 한계
- LR 변경 → 동일 eval_loss에 수렴 = **데이터가 병목**, 학습 방식으로는 더 안 올라감
- 추가 개선은 서빙 단(guided_json, temperature)에서 해결
- 발표 자료에 "v4 시도 → 트레이드오프 발견 → v3 선택 근거" 활용 가능

### 다음 할 일
- 서빙 단 최적화 (guided_json, temperature 적용)
- 발표 자료에 v4 실험 인사이트 반영
- GPT-4o 대비 정량 비교 후 sLLM 전환 최종 판단
